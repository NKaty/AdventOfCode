...
def build_robot(numbers, start):
    ...
    while True:
        if output is None:
            break
        color, pos = output
        robot_coords[(robot_position[0], robot_position[1])] = color
        update_robot_position(pos)
        output = gen.send(robot_coords.get((robot_position[0], robot_position[1]), 0))

    return len(robot_coords)
